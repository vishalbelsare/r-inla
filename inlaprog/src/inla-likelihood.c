...
double inla_compute_saturated_loglik(int thread_id, int *lcache_idx, int idx, GMRFLib_logl_tp *UNUSED(loglfunc), double *x_vec, void *arg)
{
	...
	return inla_compute_saturated_loglik_core(thread_id, lcache_idx, idx, a->loglikelihood[idx], x_vec, a->loglikelihood_arg[idx]);
}

double inla_compute_saturated_loglik_core(int thread_id, int *lcache_idx, int idx, GMRFLib_logl_tp *loglfunc, double *x_vec, void *arg)
{
	...
	SET_CACHE_IDX();
	(void) loglfunc(thread_id, &cache_idx_numa, NULL, NULL, 0, 0, NULL, NULL, NULL);
	x = xnew = xsol = 0.0;
	for (niter = 0; niter < niter_max; niter++) {
		...
		GMRFLib_2order_taylor(thread_id, &cache_idx_numa, &arr[0], &arr[1], &arr[2], NULL, 1.0, x, idx, x_vec, loglfunc, arg, &steplen,
				      &stencil);
...
#define _DIM_A  (4096L)
	...
	{
		idiv = 3;
		a[0] = ds->data_observations.gammasv_scale = Calloc(mb->predictor_ndata, double);
	}
		break;
...
{
	inla_tp *a = (inla_tp *) arg;
	return a->loglikelihood[idx] (thread_id, lcache_idx, logll, x, m, idx, x_vec, y_cdf, a->loglikelihood_arg[idx]);
}
			      ...
			      double *UNUSED(x_vec), double *y_cdf, void *arg)
{
	...
	double p1 = PREDICTOR_INVERSE_LINK_NO_SCALE(p1_intern, 0);
	double p2 = PREDICTOR_INVERSE_LINK_NO_SCALE(p2_intern, 0);
...
int loglikelihood_qcontpoisson(int thread_id, int *UNUSED(lcache_idx), double *__restrict logll, double *__restrict x, int m, int idx,
			       double *UNUSED(x_vec), double *y_cdf, void *arg)
{
	// this model is disabled
	assert(0 == 1);
	...
	GMRFLib_CACHE_SET_IDX_NO_NUMA(id);
	LINK_INIT;
	if (m > 0) {
		for (i = 0; i < m; i++) {
...
int loglikelihood_pom(int thread_id, int *UNUSED(lcache_idx), double *__restrict logll, double *__restrict x, int m, int idx, double *UNUSED(x_vec),
		      double *UNUSED(y_cdf), void *arg)
{
...
int loglikelihood_zeroinflated_negative_binomial0(int thread_id, int *UNUSED(lcache_idx), double *__restrict logll, double *__restrict x, int m,
						  int idx, double *UNUSED(x_vec), double *y_cdf, void *arg)
{
	/*
...
int loglikelihood_zeroinflated_negative_binomial1_strata2(int thread_id, int *UNUSED(lcache_idx), double *__restrict logll, double *__restrict x,
							  int m, int idx, double *UNUSED(x_vec), double *y_cdf, void *arg)
{
	/*
	 * y ~ NegativeBinomial(size, p) where E(y) = E*exp(x); same definition as in R and GSL, similar parameterisation as for the Poisson.  This version is
	 * zeroinflated type 1.
...
int inla_mix_int_quadrature_gaussian(int thread_id, int *UNUSED(lcache_idx), double **x, double **w, int *n, void *arg)
{
	Data_section_tp *ds = (Data_section_tp *) arg;
...
#pragma GCC diagnostic ignored "-Wattributes"
__attribute__((target_clones(INLA_CLONE_TARGETS "default")))
int inla_mix_int_simpson_gaussian(int thread_id, int *UNUSED(lcache_idx), double **x, double **w, int *n, void *arg)
{
...
#pragma omp critical (Name_f0fed30114239788701d492c4202a46b68cc060a)
	...
	int idx = 0;
	GMRFLib_CACHE_SET_IDX_NO_NUMA(idx);
	if (!llcache) {
...
#pragma GCC diagnostic ignored "-Wattributes"
__attribute__((target_clones(INLA_CLONE_TARGETS "default")))
int inla_mix_int_simpson_loggamma(int thread_id, int *UNUSED(lcache_idx), double **x, double **w, int *n, void *arg)
{
...
{
	inla_mix_int_simpson_loggamma(thread_id, lcache_idx, x, w, n, arg);
	for (int i = 0; i < *n; i++) {
		/*
		 * just swap the sign
		 */
			   ...
			   int (*func_simpson)(int, int *, double **, double **, int *, void *arg))
{
	...
	case MIX_INT_QUADRATURE:
	{
		if (func_quadrature) {
			 ...
			 */
			func_quadrature(thread_id, lcache_idx, &points, &weights, &np, arg);
		} else {
			assert(0 == 1);
		}
	}
	...
	case MIX_INT_SIMPSON:
	{
		if (func_simpson) {
			 ...
			 * we can get back few points, as small weights are removed
			 */
			func_simpson(thread_id, lcache_idx, &points, &weights, &np, arg);
		} else {
			assert(0 == 1);
		}
	}
	...
	if (m > 0) {
		...
		assert(kk == mm);
		ds->mix_loglikelihood(thread_id, lcache_idx, ll, xx, mm, idx, x_vec, NULL, arg);
		for (i = 0, kk = 0; i < m; i++) {
			for (k = 0; k < np; k++) {
			  ...
			  double *UNUSED(x_vec), double *y_cdf, void *arg)
{
	...
	Data_section_tp *ds = (Data_section_tp *) arg;
	double y = ds->data_observations.y[idx];
	double s = (ds->data_observations.gammasv_scale ? ds->data_observations.gammasv_scale[idx] : 1.0);
	double phi_param = map_exp_forward(ds->data_observations.gammasv_log_prec[thread_id][0], MAP_FORWARD, NULL);
	double phi = phi_param * s;
...
#define _LOGGAMMA_INT(xx_) my_gsl_sf_lnfact(((xx_) - 1))
	...
	if (m > 0) {
		...
		if (pp < p_upper) {
			...
			xx[1] = PREDICTOR_LINK(p_upper, off);
			xx[0] = xx[1] - h;
			xx[2] = xx[1] + h;
			for (int i = 0; i < 3; i++) {
		...
		for (int i = 0; i < -m; i++) {
			...
			if (n > 500 && give_warning) {
				give_warning = 0;
				printf("\n*** Warning ***  Version [%s]", __GMRFLib_symbol_to_string(GITCOMMIT));
...
int loglikelihood_generic_surv(int thread_id, int *lcache_idx, double *__restrict logll, double *__restrict x, int m, int idx, double *x_vec,
			       double *y_cdf, void *arg, GMRFLib_logl_tp *loglfun)
{
...
#define SAFEGUARD(value_)						\
	...
	if (m > 0) {
		...
		if (have_truncation) {
			loglfun(thread_id, lcache_idx, F_trunc, x, -m, idx, x_vec, &truncation, arg);
			SAFEGUARD(F_trunc);
#pragma omp simd
			for (int i = 0; i < m; i++) {
		...
		{
			loglfun(thread_id, lcache_idx, lf, x, m, idx, x_vec, NULL, arg);
			if (have_truncation) {
#pragma omp simd
				for (int i = 0; i < m; i++) {
...
#pragma omp simd
		...
		case SURV_EVENT_RIGHT:
		{
			if (ISNONZERO(lower)) {
				...
				loglfun(thread_id, lcache_idx, F_lower, x, -m, idx, x_vec, &lower, arg);
				SAFEGUARD(F_lower);
			}
...
#pragma omp simd
		...
		case SURV_EVENT_LEFT:
		{
			if (!ISINF(upper)) {
				...
				loglfun(thread_id, lcache_idx, F_upper, x, -m, idx, x_vec, &upper, arg);
				SAFEGUARD(F_upper);
			}
		       ...
		       double *UNUSED(x_vec), double *UNUSED(y_cdf), void *arg)
{
	...
	if (m > 0) {
		...
		for (int i = 0; i < m; i++) {
			...
			double jac = PREDICTOR_INVERSE_LINK_PLAIN_DERIV(yp - lp) * jac_part;
			logll[i] = -lc + prec * cos(z) + log(jac);
		}
	} else {
		GMRFLib_dfill(-m, 0.0, logll);
	}
